...
dataset_relative_path = 'grouplens/ratings.dat'
...
def run(data_folder):
    ...
    kg = load_kg(kg_path)
    dataset = load_movielens(dataset_path)[['u', 'i', 'r']]
    linking = load_linking(linking_path, names=['i', 'n', 'e'])
    ...
    # data filtering
    binarizer = Binarize(dataset=dataset, threshold=3)
...
if __name__ == "__main__":
    run(data_folder='/'.join(os.getcwd().split("/")[:-1]) + "/data/movielens")